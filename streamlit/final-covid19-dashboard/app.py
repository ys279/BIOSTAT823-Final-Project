        ...
        if g_or_us == "Global":
            ...
            if active:
                g_f = px.scatter_mapbox(g_df,  
                                        ...
                                        hover_data=['Confirmed', 'Deaths', 'Recovered'],hover_name='Combined_Key'
                                       )
            ...
            if ratio:
                #calculate ratio
                g_df["Case-Fatality Ratio"] = round((g_df["Deaths"]/(g_df["Confirmed"]+g_df["Recovered"]))*100, 2)
                ...
                with st.beta_expander("What is Case-Fatality Ratio?"):
                    st.markdown("""
                    The **Case-Fatality Ratio (CFR)** is the proportion of individuals diagnosed with a disease who die from that disease and is therefore a measure of severity among detected cases:  
                    ***CFR = Number of deaths/(Number of deaths + Number of recovered) x 100***
                    """)
                st.plotly_chart(r_f)
