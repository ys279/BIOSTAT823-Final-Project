...
if session == "Map":
    ...
    if mapplots == "Spread of COVID-19":
        ...
        start_date = st.sidebar.date_input('Start date (no late than 2020/1/22)', datetime(2020, 3,22))
        end_date = st.sidebar.date_input('End date', datetime(2020,5,3))
        ...
        if st.sidebar.checkbox("Show me the spread ", value=True):
            ...
            animate = px.scatter_mapbox(full_table[mask],lat="Lat", lon="Long",
                                        ...
                                        title = 'Spread of COVID-19 from {start} to {end}'.format(start = start_date.strftime("%Y-%m-%d"), end = end_date.strftime("%Y-%m-%d"))
                                       )
